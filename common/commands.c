...
void commands_process_packet(unsigned char *data, unsigned int len,
	...
	if (receiver_id == main_id || receiver_id == ID_ALL || receiver_id == ID_CAR_CLIENT) {
		...
		if (VEHICLE_TYPE == VEHICLE_TYPE_COPTER) {
			switch(packet_id) {
			case CMD_MR_GET_STATE: {
				...
				buffer_append_float32_auto(m_send_buffer, 0.0, &send_index); // 72
				...
				buffer_append_int32(m_send_buffer, time_today_get_ms(), &send_index); // 92
				commands_send_packet(m_send_buffer, send_index);
			} break;
