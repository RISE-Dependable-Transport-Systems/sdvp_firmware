 ...
 * Handles idle interrupts depending on configured
...
void ublox_init(void) {
	...
	ublox_cfg_msg(UBX_CLASS_NMEA, UBX_NMEA_GSV, 0);
...
static void ubx_decode_ack(uint8_t *msg, int len) {
	...
	if (ack_wait_tp) {
		chEvtSignal(ack_wait_tp, (eventmask_t)1);
	}
}
...
static void ubx_decode_nak(uint8_t *msg, int len) {
	...
	if (ack_wait_tp) {
		chEvtSignal(ack_wait_tp, (eventmask_t)2);
	}
}
